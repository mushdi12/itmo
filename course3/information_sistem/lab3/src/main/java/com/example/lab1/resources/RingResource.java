...
public class RingResource {
    ...
    public Response list() {
        try {
            ...
            e.printStackTrace(); // Выведет в логи сервера
            return Response.status(Response.Status.INTERNAL_SERVER_ERROR)
                    .entity(Map.of("error", "Ошибка при получении списка колец: " + e.getMessage()))
                    .build();
        }
    }
    ...
    public Response create(RingDto dto, @Context UriInfo uriInfo) {
        try {
            ...
            e.printStackTrace();
            return Response.status(Response.Status.INTERNAL_SERVER_ERROR)
                    .entity(Map.of("error", "Ошибка при создании кольца: " + e.getMessage()))
                    .build();
        }
    }
