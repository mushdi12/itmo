...
public class BookCreatureResource {
    ...
    public Response list() {
        try {
            List<BookCreature> creatures = bookCreatureService.findAllBookCreatures();
            // Возвращаем пустой массив, а не 404, если список пуст
            return Response.ok(creatures).build();
        } catch (Exception e) {
            e.printStackTrace();
            return Response.status(Response.Status.INTERNAL_SERVER_ERROR)
                    .entity(Map.of("error", "Ошибка при получении списка существ: " + e.getMessage()))
                    .build();
        }
    }
