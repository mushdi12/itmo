  ...
  return (
    <div className="card">
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '1rem' }}>
        <h3 style={{ margin: 0, color: '#4a5568' }}>
          ...
          {typeof creature.id !== 'undefined' && (
            <span style={{ marginLeft: '0.5rem', color: '#a0aec0', fontSize: '0.9rem' }}># {creature.id}</span>
          )}
        </h3>
      ...
      <div style={{ marginTop: '1rem', fontSize: '0.9rem', color: '#718096' }}>
        <p><strong>📍 Местоположение:</strong> {creature.creatureLocation.name}</p>
        <p><strong>🗺️ Координаты:</strong> ({creature.coordinates.x}, {creature.coordinates.y})</p>
        <p><strong>💍 Кольцо:</strong> {creature.ring.name} {creature.ring.power && `(сила: ${creature.ring.power})`}</p>
        <p><strong>📅 Создано:</strong> {formatDate(creature.creationDate)}</p>
      </div>
