  ...
  return (
    <div className="card">
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '1rem' }}>
        <h3 style={{ margin: 0, color: '#4a5568' }}>
          ...
          {typeof city.id !== 'undefined' && (
            <span style={{ marginLeft: '0.5rem', color: '#a0aec0', fontSize: '0.9rem' }}># {city.id}</span>
          )}
      ...
      <div style={{ marginTop: '1rem', fontSize: '0.9rem', color: '#718096' }}>
        <p>
          <strong>{getGovernorEmoji(city.governor)} Правитель:</strong> {city.governor}
        </p>

        <p><strong>📅 Основан:</strong> {formatDate(city.establishmentDate)}</p>
      </div>
