...
class AuthControllerTest {
    ...
    // Вспомогательный метод для мокирования статического JwtUtil.getUsernameFromToken
    private void mockStaticJwtUtil(String returnUsername) {
        try (var mockedStatic = Mockito.mockStatic(com.example.backend.auth.jwt.JwtUtil.class)) {
            ...
            // Чтобы mockedStatic не закрывался сразу, можно передать в тест, но в простом тесте подойдет так
            // В тестах выше вызовы выполняются сразу, поэтому с try-with-resources все ок.
        }
    }
}